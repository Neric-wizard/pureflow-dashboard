...
export default function SafetyGauge({ score, lastReading = 75 }) {
  ...
  const smartInsight =
    score < 50 ? '⚠️ Immediate filtration recommended' :
  ...
  return (
    <div className="relative group" role="region" aria-label="Safety Score Gauge — System health indicator">
      ...
      <div className="relative bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 shadow-[0_25px_60px_-15px_rgb(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-2xl overflow-hidden">
        ...
        <div className="absolute top-3 right-4 text-[7px] font-mono text-gray-500 animate-pulse">
          ● Updated just now
        </div>
        ...
        <div className="relative flex items-center gap-6 flex-wrap">
          ...
          <div className="flex gap-5">
            {forecast.map(({ day, risk, color }, idx) => (
              <div key={idx} className="flex flex-col items-center" role="group" aria-label={`${day} risk: ${risk}`}>
                ...
                <span className="font-mono text-[9px] font-semibold mt-1.5 tracking-wider" style={{ color }}>{risk}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-3 text-[8px]">
              <span className="text-blue-400 tabular-nums" title="Turbidity contributes to safety score">💧 Turbidity: {turbidityContribution}%</span>
              <span className="text-amber-400 tabular-nums" title="pH contributes to safety score">🧪 pH: {pHContribution}%</span>
            </div>
